...
def random_arr_graph(arr_random_block, arr_random_quick, arr_random_selection):
    lengths = [i for i in range(100, 1301, 100)]
...
def test_time():
    ...
    for i in range(100, 1301, 100):
        ...
        arr_random_quick.append(get_time(quick_sort, get_random_array, i))
        arr_random_selection.append(get_time(selection_sort, get_random_array, i))

        arr_reversed_block.append(get_time(block_sort, get_reversed_array, i))
        arr_reversed_quick.append(get_time(quick_sort, get_reversed_array, i))
        arr_reversed_selection.append(get_time(selection_sort, get_reversed_array, i))

        arr_sorted_block.append(get_time(block_sort, get_sorted_array, i))
        arr_sorted_quick.append(get_time(quick_sort, get_sorted_array, i))
        arr_sorted_selection.append(get_time(selection_sort, get_sorted_array, i))

        print("Выполнено", i // 100, "/ 13 замеров\n")
    ...
    for i in range(10):
        print(" %3d | %8f | %8f | %8f\n" %(i * 100, arr_random_block[i] * 1000, arr_random_quick[i] * 1000, arr_random_selection[i] * 1000)) 
    print("\n")

    print("ТАБЛИЦА ДЛЯ АЛГОРИТМОВ БЛОЧНОЙ, БЫСТРОЙ СОРТИРОВОК, СОРТИРОВКИ ВЫБОРОМ (ОТСОРТИРОВАННЫЙ В ОБРАТНОМ ПОРЯДКЕ МАССИВ)")
    print(" %3s | %8s | %8s | %8s\n" %("i", "Блочная", "Быстрая", "Выбором"))
    for i in range(10):
        print(" %3d | %8f | %8f | %8f\n" %(i * 100, arr_reversed_block[i] * 1000, arr_reversed_quick[i] * 1000, arr_reversed_selection[i] * 1000)) 
    print("\n")

    print("ТАБЛИЦА ДЛЯ АЛГОРИТМОВ БЛОЧНОЙ, БЫСТРОЙ СОРТИРОВОК, СОРТИРОВКИ ВЫБОРОМ (ОТСОРТИРОВАННЫЙ МАССИВ)")
    print(" %3s | %8s | %8s | %8s\n" %("i", "Блочная", "Быстрая", "Выбором"))
    for i in range(10):
        print(" %3d | %8f | %8f | %8f\n" %(i * 100, arr_sorted_block[i] * 1000, arr_sorted_quick[i] * 1000, arr_sorted_selection[i] * 1000)) 
    print("\n")
